...
│   │   └── Providers/
│   │       └── index.tsx            # Auth provider wrapper
│   ├── hooks/
...
- ✅ `src/theme/Root.tsx` - Root wrapper (AuthProvider)
- ✅ `src/theme/DocItem/Layout/index.tsx` - Swizzled layout

**How It Works:**
1. Docusaurus loads `Root.tsx` first, wrapping the entire app
2. `Root.tsx` provides `AuthProvider` context to all components
3. `DocItem/Layout` is automatically used for all doc pages
4. `PersonalizeButton` has access to auth context via `useAuth()`
...
2. **Token Storage:** Frontend stores user data + token in localStorage
3. **Auth Context:** `AuthProvider` makes user data available via `useAuth()` hook
4. **Button Visibility:** `PersonalizeButton` only renders if `isAuthenticated === true`
...
### Button Not Showing
- Check if user is logged in (`console.log(useAuth())`)
- Verify `AuthProvider` is wrapping the app (check `Root.tsx`)
...
- [x] Root.tsx provides AuthProvider
- [x] No additional config changes needed
